...
public function printTicket($order_id, $cash = null, $change = null)
{
    ...
    try {
        ...
        // Título según Estado Dian
        $printer->setTextSize(1, 2);
        $printer->setJustification(Printer::JUSTIFY_CENTER);
        if ($estadoDian === 'Recibida') {
            $printer->text("Factura Electronica\n");
        } else {
            $printer->text("Factura\n");
        }
        ...
        // Impresión de datos del cliente
        $printer->text("\n-----------------------------------------\n");
        if (!empty($order->client->first_name)) {
            $printer->text(sprintf('%-16s %-25s', 'Primer Nombre:', $order->client->first_name) . "\n");
        }
        if (!empty($order->client->second_name)) {
            $printer->text(sprintf('%-16s %-25s', 'Segundo Nombre:', $order->client->second_name) . "\n");
        }
        if (!empty($order->client->first_lastname)) {
            $printer->text(sprintf('%-16s %-25s', 'Primer Apellido:', $order->client->first_lastname) . "\n");
        }
        if (!empty($order->client->second_lastname)) {
            $printer->text(sprintf('%-16s %-25s', 'Segundo Apellido:', $order->client->second_lastname) . "\n");
        }
        if (!empty($order->client->razon_social)) {
            $printer->text(sprintf('%-16s %-25s', 'Razón Social:', $order->client->razon_social) . "\n");
        }
        if (!empty($order->client->document)) {
            $printer->text(sprintf('%-16s %-25s', 'Documento:', $order->client->document) . "\n");
        }
        if (!empty($order->client->div_verification)) {
            $printer->text(sprintf('%-16s %-25s', 'Div. Verif.:', $order->client->div_verification) . "\n");
        }
        ...
        if ($order->client->municipality) {
            $municipio = $order->client->municipality->name;
            if (!empty($municipio)) {
                $printer->text(sprintf('%-16s %-25s', 'Municipio:', $municipio) . "\n");
            }
        }
        if ($order->client->clientTribute) {
            $tributo = $order->client->clientTribute->name;
            if (!empty($tributo)) {
                $printer->text(sprintf('%-16s %-25s', 'Tributo Cliente:', $tributo) . "\n");
            }
        }
        if ($order->client->identityDocumentType) {
            $tipoDoc = $order->client->identityDocumentType->name;
            if (!empty($tipoDoc)) {
                $printer->text(sprintf('%-16s %-25s', 'Tipo Documento:', $tipoDoc) . "\n");
            }
        }
        ...
        foreach ($order_details as $df) {
            $line = sprintf(
                '%-22s %8.2f %10.2f ',
                '-' . mb_strimwidth($df->product, 0, 21, ''),
                $df->quantity,
                $df->price_tax_inc_total
            );
            $total += $df->price_tax_inc_total;
            $printer->text($line . "\n");
        }
        ...
        $printer->text("Escanea este código para acceder a tu factura \n\n");
        $printer->text("Tecnoplus\n");
        $printer->text("www.tecnoplus.online\n");
        $printer->cut();
        $printer->pulse();
        $printer->close();
...
public function printUpdateComanda(Order $order, array $changes)
{
    ...
    public function printTicketRecently($order_id, $listProducts = null, $reprint = false)
    {
        ...
        foreach ($productosPorZona as $zonaData) {
            ...
            try {
                ...
                $printer->text("========================================\n");
                $printer->setEmphasis(true);
                $printer->text(sprintf('%-20s %4s %8s', 'ARTÍCULO', 'CANT', 'VALOR') . "\n");
                $printer->setEmphasis(false);
                $printer->text("========================================\n");

                // Imprimir cada producto y sus observaciones
                foreach ($zonaData['productos'] as $producto) {
                    $maxWidthText = 25; // Reducido para aprovechar mejor el espacio
                    $cantidad     = intval($producto->quantity);
                    $valorTotal   = $producto->price_tax_inc_total;
                    $wrappedText  = wordwrap($producto->product, $maxWidthText);
                    $textLines    = explode("\n", $wrappedText);
                    ...
                    // Nombre, cantidad y valor
                    foreach ($textLines as $i => $line) {
                        $qtyDisplay   = $i === 0 ? sprintf('%4s', $cantidad) : str_repeat(' ', 4);
                        $valDisplay   = $i === 0 ? sprintf('%8.2f', $valorTotal) : str_repeat(' ', 8);
                        $printer->text(sprintf("%-{$maxWidthText}s %s %s\n", $line, $qtyDisplay, $valDisplay));
                    }
                    ...
                    if (! empty($producto->observaciones)) {
                        ...
                        $obsLines   = explode("\n", $wrappedObs);
                        foreach ($obsLines as $obsLine) {
                            // Indentación mínima para ajustar al borde
                            $printer->text(sprintf("-> %-{$maxWidthText}s\n", $obsLine));
                        }
                    }
                ...
                // Observaciones de la orden en negrita
                if (! empty($order->observations)) {
                    $printer->feed(1);
                    $printer->setEmphasis(true);
  ...
  //sin precios
    public function printTicketRecently($order_id, $listProducts = null, $reprint = false)
    {
        ...
        // Agrupamos productos por zona y asignamos cantidad y observaciones
        foreach ($order->products as $product) {
            foreach ($product->zones as $zone) {
                // Ajustar cantidad/observaciones según edición o creación
                if ($listProducts) {
                    $matched = null;
                    foreach ($listProducts as $newProduct) {
                        if ($product->id == $newProduct['product_id']) {
        ...
        foreach ($productosPorZona as $zonaData) {
            $zone        = $zonaData['zone'];
            $pos_printer = $zone->printer;
            if (empty($zonaData['productos']) || ! $pos_printer) {
                continue;
            }
            ...
            try {
                ...
                // Encabezado de lista (solo ARTÍCULO y CANTIDAD) en texto más grande y negrita
                $printer->setTextSize(1, 2);
                $printer->text("========================================\n");
                $printer->setEmphasis(true);
                $printer->text(sprintf('%-25s %4s', 'ARTÍCULO', 'CANT') . "\n");
                $printer->setEmphasis(false);
                $printer->text("========================================\n");
                // Volvemos a tamaño normal para observaciones y separadores
                $printer->setTextSize(1, 1);
                ...
                foreach ($zonaData['productos'] as $producto) {
                    ...
                    // Nombre y cantidad en negrita y ligeramente más grande
                    $printer->setTextSize(1, 2);
                    $printer->setEmphasis(true);
                    foreach ($textLines as $i => $line) {
                        $qtyDisplay = $i === 0 ? sprintf('%4s', $cantidad) : str_repeat(' ', 4);
                    ...
                    // Observaciones, si existen (tamaño normal)
                    if (! empty($producto->observaciones)) {
                        $wrappedObs = wordwrap($producto->observaciones, $maxWidthText);
                        $obsLines   = explode("\n", $wrappedObs);
                        foreach ($obsLines as $obsLine) {
                ...
                Log::error("Error al imprimir en zona {$zone->zone}: " . $e->getMessage());
            }
        }
    ...
    /**
     * Imprime la comanda de reserva, con el mismo diseño que printTicketRecently
     * pero destacando el título "RESERVA" y la fecha/hora de la reserva.
     *
     * @param  int   $order_id
     * @param  array $listProducts  (igual que en printTicketRecently, opcional)
     * @param  bool  $reprint       (si es reimpresión)
     * @return \Illuminate\Http\RedirectResponse
     */
    public function printReservationTicket($order_id, $listProducts = null, $reprint = false)
    {
        // 1) Cargar orden, usuario y configuración
        $order         = Order::with('detailOrders', 'products.zones', 'table', 'user')->findOrFail($order_id);
        $system_user   = $order->user;
        $configuration = new Configuration();
        $company       = $configuration->select()->first();

        // 2) Agrupar productos por zona
        $productosPorZona = [];
        foreach ($order->products as $product) {
            foreach ($product->zones as $zone) {
                // Ajustar cantidad/observaciones según edición o creación
        ...
        foreach ($productosPorZona as $zonaData) {
            ...
            try {
                ...
                $printer->setTextSize(3, 3);
                $printer->setEmphasis(true);
                $printer->text("RESERVA\n");
                $printer->setEmphasis(false);
                $printer->feed(1);

                // Fecha y hora de la reserva, centrado y tamaño destacado
                $reservationAt = Carbon::parse($order->reservation_at);
                $printer->setTextSize(2, 2);
                $printer->text("Fecha: " . $reservationAt->format('Y-m-d') . "\n");
                $printer->text("Hora : " . $reservationAt->format('H:i') . "\n");
                $printer->feed(1);
                ...
                // Mesero y mesa, igual que en comanda normal
                $printer->setTextSize(2, 2);
                $printer->setEmphasis(true);
                $printer->text("Mesero(a): " . $system_user->name . "\n");
                $printer->setEmphasis(false);
                ...
                foreach ($zonaData['productos'] as $producto) {
                    ...
                    // Nombre y cantidad en negrita y tamaño medio
                    $printer->setTextSize(1, 2);
                    $printer->setEmphasis(true);
                    foreach ($textLines as $i => $line) {
                        $qtyDisplay = $i === 0 ? sprintf('%4s', $cantidad) : str_repeat(' ', 4);
                ...
                Log::error("Error al imprimir reserva en zona {$zone->zone}: " . $e->getMessage());
            }
        }

        return redirect()->back()->with('mensaje', 'Comanda de reserva impresa');
    }
