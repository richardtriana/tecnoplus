      ...
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <p>Selecciona los productos que quieres trasladar a la nueva cuenta:</p>
            <ul class="list-group">
              <li
                v-for="p in productsOrderList"
                :key="'split-' + p.id"
              ...
              >
                ...
                <span class="flex-fill">
                  {{ p.product }} — Cantidad: {{ p.quantity }} — Total:
                  {{ ((p.price_tax_inc * p.quantity) - (p.price_tax_exc * p.quantity * (p.discount_percentage/100)))
                      .toLocaleString('es-CO', { minimumFractionDigits: 2 }) }}
                </span>
              </li>
            </ul>
...
methods: {
  ...
  onBoxSelected({ boxId, voucherId }) {
    ...
    // ——> parche: si la caja está cerrada, mostramos alerta y redirigimos
    ...
    // Si la caja está abierta, continúa con la lógica habitual
    this.listItemsOrder();
  },
    loadPaymentForms() {
    ...
    async confirmSplit() {
      try {
        const { data } = await axios.post(
          ...
          { detail_ids: this.selectedSplitDetails },
          this.$root.config
        );
