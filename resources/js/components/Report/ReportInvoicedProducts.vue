    ...
    <div class="page w-100">
      ...
      <div class="page-search mx-2 my-2 border p-2">
        <div class="form-row">
          ...
          <!-- Filtro para el estado de la orden: Todas, Activas o Eliminadas -->
          <div class="form-group col-md-3">
            <label for="order_delete">Estado de la Orden</label>
            <v-select 
          ...
          <div class="form-group col-3">
            ...
            <input type="number" step="any" class="form-control" id="nro_results" placeholder="Resultados" v-model="filter.nro_results" />
          </div>
          ...
          <div class="col my-4 col-3">
            <download-excel class="btn btn-outline-success mr-2 btn-block" :fields="json_fields" :data="List.data"
              name="invoiced-products.xls" type="xls">
  ...
  export default {
    data() {
      return {
        ...
        filter: {
          ...
          product: "",
          category: "",
          order_delete: "all", // Valores posibles: 'all', 'active', 'deleted'
          nro_results: 15
        },
        categoriesList: [],
        orderDeleteOptions: [
    ...
    methods: {
      getOrders(page = 1) {
        ...
        axios
          .get(`api/reports/invoiced-products?page=${page}&${params}`, this.$root.config)
          .then(response => {
      ...
      getTicket() {
        axios
          .post(`api/reports-ticket/invoiced-products`, { data: this.List.data }, this.$root.config)
          .then(() => {
